    ...
    return (
        <Container className="mt-3">
            ...
            <Row className="d-flex justify-content-center mt-4">
                {!props.loadingTickets ? <TicketList user={props.user} tickets={props.tickets.filter(t => ["IN_PROGRESS", "CLOSED", "RESOLVED"].includes(t.status))} messages={props.messages} nameFilter={nameFilter} getMessages={props.getMessages} stopTicket={props.stopTicket}/> : <Spinner  variant="primary"/>}
            </Row>
        </Container>
    )
}
